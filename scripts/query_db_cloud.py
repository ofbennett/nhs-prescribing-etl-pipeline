...
def run_query_and_save_to_s3(date,med_num,med,conn,cur):
    if med == 'All':
        query = q_all_meds.format(year=date[:4],month=date[5:])
    else:
...
def download_results_from_s3(date,med_num,med,s3):
...
def process_downloaded_files(date,med_num,med):
...
def main():
    ...
    for date in dates:
        for i, med in enumerate(meds):
            run_query_and_save_to_s3(date,i,med,conn,cur)
    cur.close()
    conn.close()
    ...
    for date in dates:
        for i, med in enumerate(meds):
            download_results_from_s3(date,i,med,s3)
    
    print("Processing downloaded files...")
    for date in dates:
        for i, med in enumerate(meds):
            process_downloaded_files(date,i,med)
