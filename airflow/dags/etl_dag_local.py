...
from helpers import sql_queries_local
...
drop_all_tables = PostgresOperator(
    ...
    sql=sql_queries_local.drop_all_tables
)
...
create_all_tables_if_not_exist = PostgresOperator(
    ...
    dag=dag,
    postgres_conn_id="my_postgres_conn",
    sql=sql_queries_local.create_all_tables
)
...
pres_staging_table_populate = PostgresOperator(
    ...
    dag=dag,
    postgres_conn_id="my_postgres_conn",
    sql=sql_queries_local.pres_staging_table_populate
)
...
gp_prac_staging_table_populate = PostgresOperator(
    ...
    dag=dag,
    postgres_conn_id="my_postgres_conn",
    sql=sql_queries_local.gp_prac_staging_table_populate
)
...
bnf_info_staging_table_populate = PostgresOperator(
    ...
    dag=dag,
    postgres_conn_id="my_postgres_conn",
    sql=sql_queries_local.bnf_info_staging_table_populate
)
...
postcode_info_staging_table_populate = PostgresOperator(
    ...
    dag=dag,
    postgres_conn_id="my_postgres_conn",
    sql=sql_queries_local.postcode_info_staging_table_populate
)
...
pres_fact_table_insert = PostgresOperator(
    ...
    dag=dag,
    postgres_conn_id="my_postgres_conn",
    sql=sql_queries_local.pres_fact_table_insert
)
...
gp_pracs_dim_table_insert = PostgresOperator(
    ...
    dag=dag,
    postgres_conn_id="my_postgres_conn",
    sql=sql_queries_local.gp_pracs_dim_table_insert
)
...
bnf_info_dim_table_insert = PostgresOperator(
    ...
    dag=dag,
    postgres_conn_id="my_postgres_conn",
    sql=sql_queries_local.bnf_info_dim_table_insert
)

run_quality_checks = DataQualityOperator(
    task_id='Run_data_quality_checks',
    dag=dag,
    quality_checks=sql_queries_local.quality_tests
)
